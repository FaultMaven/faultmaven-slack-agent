...
| `shortcuts: [fm_investigate_message]` + `interactivity` + `commands` | the message shortcut (named just `Ask` — Slack appends the app name, so it shows as **Ask FaultMaven**; shortcuts need the `commands` scope) |
